...
async function executeQuery(query: string, customerId: string, accessToken: string, devToken: string) {
  ...
  try {
    ...
    throw new Error(`Failed to parse response: ${rawText.substring(0, 500)}`);
  }
...
function getDateRange(date_preset?: string, date_range?: { start: string; end: string }) {
  if (date_range?.start && date_range?.end) return { start: date_range.start, end: date_range.end };
  ...
  switch (date_preset) {
    ...
    case "yesterday":
      return { start: fmt(sub(1)), end: fmt(sub(1)) };
    case "last_7d":
    ...
    case "last_365d":
      return { start: fmt(sub(365)), end: fmt(sub(1)) };
    case "this_month": {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Fetching Google Ads (breakdown: ${breakdown || "aggregate"}) for customer: ${customerId}, ${dateRange.start} to ${dateRange.end}`,
    );
    ...
    if (breakdown === "ad_group") {
      const query = `SELECT ad_group.name, ad_group.id, ad_group.status, campaign.name, metrics.cost_micros, metrics.impressions, metrics.clicks, metrics.conversions, metrics.cost_per_conversion FROM ad_group WHERE ${dateFilter} AND campaign.status != 'REMOVED' AND ad_group.status != 'REMOVED' ORDER BY metrics.cost_micros DESC LIMIT 50`;
      const results = await executeQuery(query, customerId, accessToken, GOOGLE_DEVELOPER_TOKEN);
      ...
      return new Response(JSON.stringify({ ad_groups: adGroups }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (breakdown === "keyword") {
      const query = `SELECT ad_group_criterion.keyword.text, ad_group_criterion.keyword.match_type, ad_group_criterion.status, campaign.name, metrics.cost_micros, metrics.impressions, metrics.clicks, metrics.conversions, metrics.ctr, metrics.average_cpc, metrics.cost_per_conversion FROM keyword_view WHERE ${dateFilter} AND campaign.status != 'REMOVED' ORDER BY metrics.cost_micros DESC LIMIT 50`;
      const results = await executeQuery(query, customerId, accessToken, GOOGLE_DEVELOPER_TOKEN);
    ...
    if (breakdown === "day_of_week") {
      ...
      return new Response(JSON.stringify({ day_of_week: dayOfWeek }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
